...
async def analisar_simples(file: UploadFile = File(...), tipo_delineamento: str = Form("dbc")):
    try:
        ...
        if tipo_delineamento == "dql":
            ...
            f_trat, f_linha, f_col = qm_trat/qm_res if qm_res>0 else 0, qm_linha/qm_res if qm_res>0 else 0, qm_col/qm_res if qm_res>0 else 0
            ...
            nT, nR = len(df['Trat'].unique()), len(df['Bloco'].unique()) if tipo_delineamento == "dbc" else 1
            ...
            if tipo_delineamento == "dbc":
                ...
                qm_trat, qm_bloc, qm_res = sq_trat/gl_trat, sq_bloc/gl_bloc, sq_res/gl_res if gl_res>0 else 0
...
async def analisar_fatorial(file: UploadFile = File(...), tipo_teste: str = Form("anova"), modelo_regr: str = Form("linear"), testemunha: str = Form("")):
    try:
        ...
        # === QUADRO DA ANOVA ===
        if tipo_teste == "anova":
            return {"status": "sucesso", "cv": round((np.sqrt(qm_res)/mg)*100, 2), "anova": [
        ...
        if "regr_" in tipo_teste:
            if pd.to_numeric(df[fator], errors='coerce').isna().any(): return {"status": "erro", "mensagem": "Regressão exige doses numéricas."}
            x, y = np.array(medias.index, dtype=float), np.array(medias.values, dtype=float)
            fig, ax = plt.subplots(figsize=(8, 5)); ax.scatter(x, y, color='black', label='Médias')
            x_plot = np.linspace(min(x), max(x), 100)
            
            if modelo_regr == "linear":
                s, i, r, _, _ = stats.linregress(x, y)
                ax.plot(x_plot, i + s * x_plot, color='blue'); eq, r2 = f"y = {i:.4f} + {s:.4f}x", f"{r**2:.4f}"
            elif modelo_regr == "quadratica":
                c = np.polyfit(x, y, 2); p = np.poly1d(c); ax.plot(x_plot, p(x_plot), color='red')
                eq, r2 = f"y = {c[2]:.4f} + ({c[1]:.4f})x + ({c[0]:.4f})x²", f"{1 - (np.sum((y - p(x))**2) / np.sum((y - np.mean(y))**2)):.4f}"
            elif modelo_regr == "logaritmica":
                s, i, r, _, _ = stats.linregress(np.log(x), y); ax.plot(x_plot, i + s * np.log(x_plot), color='green')
                eq, r2 = f"y = {i:.4f} + {s:.4f} * ln(x)", f"{r**2:.4f}"
            elif modelo_regr == "exponencial":
                s, i, r, _, _ = stats.linregress(x, np.log(y)); ax.plot(x_plot, np.exp(i) * np.exp(s * x_plot), color='purple')
                eq, r2 = f"y = {np.exp(i):.4f} * e^({s:.4f}x)", f"{r**2:.4f}"

            ax.set_title(f"Regressão - Fator {fator}"); ax.grid(True)
