...
async def analisar_simples(file: UploadFile = File(...), tipo_delineamento: str = Form("dbc"), tipo_teste: str = Form("anova"), modelo_regr: str = Form("linear"), testemunha: str = Form("")):
    try:
        ...
        if tipo_delineamento == "dql":
            ...
            anova = [
                {"FV": "Tratamentos", "GL": gl_trat, "SQ": round(sq_trat,2), "QM": round(qm_trat,2), "F Calc": round(qm_trat/qm_res if qm_res>0 else 0,2), "F Tab": calcular_f_tab(gl_trat, gl_res), "Sig": determinar_sig_texto(1 - stats.f.cdf(qm_trat/qm_res if qm_res>0 else 0, gl_trat, gl_res), gl_trat, gl_res)},
                {"FV": "Linhas", "GL": gl_linha, "SQ": round(sq_linha,2), "QM": round(sq_linha/gl_linha,2), "F Calc": round((sq_linha/gl_linha)/qm_res if qm_res>0 else 0,2), "F Tab": calcular_f_tab(gl_linha, gl_res), "Sig": determinar_sig_texto(1 - stats.f.cdf((sq_linha/gl_linha)/qm_res if qm_res>0 else 0, gl_linha, gl_res), gl_linha, gl_res)},
                {"FV": "Colunas", "GL": gl_col, "SQ": round(sq_col,2), "QM": round(sq_col/gl_col,2), "F Calc": round((sq_col/gl_col)/qm_res if qm_res>0 else 0,2), "F Tab": calcular_f_tab(gl_col, gl_res), "Sig": determinar_sig_texto(1 - stats.f.cdf((sq_col/gl_col)/qm_res if qm_res>0 else 0, gl_col, gl_res), gl_col, gl_res)},
        ...
        if "regr" in tipo_teste:
            ...
            ax.scatter(x, y, color='black', label='Médias', zorder=5)
            ...
            anova_reg = [
                {"FV": f"Regressão ({modelo_regr.capitalize()})", "GL": gl_reg, "SQ": round(sq_reg_raw, 2), "QM": round(qm_reg, 2), "F Calc": round(f_reg, 2), "F Tab": calcular_f_tab(gl_reg, gl_res), "Sig": determinar_sig_texto(1 - stats.f.cdf(f_reg, gl_reg, gl_res), gl_reg, gl_res)},
                {"FV": "Desvios da Regressão", "GL": gl_desvio, "SQ": round(sq_desvio_raw, 2), "QM": round(qm_desvio, 2), "F Calc": round(f_desvio, 2), "F Tab": calcular_f_tab(gl_desvio, gl_res), "Sig": determinar_sig_texto(1 - stats.f.cdf(f_desvio, gl_desvio, gl_res), gl_desvio, gl_res)}
            ]
            ax.set_title("Ajuste de Regressão")
            ax.grid(True, linestyle='--', alpha=0.6); ax.legend()
            b_png, b_pdf = io.BytesIO(), io.BytesIO()
            fig.savefig(b_png, format="png", bbox_inches='tight'); fig.savefig(b_pdf, format="pdf", bbox_inches='tight')
            plt.close(fig)
        ...
        if "tukey" in tipo_teste:
            q_crit = qsturng(0.95, nT, gl_res); dms = q_crit * ep
        ...
        if "duncan" in tipo_teste:
            rp_duncan = {p: round(qsturng((0.95)**(p-1), p, gl_res) * ep, 4) for p in range(2, nT + 1)}
        ...
        if "dunnett" in tipo_teste:
            ...
            dms_dunnett = stats.t.ppf(1 - (0.05 / (2 * (nT - 1))), gl_res) * ep * np.sqrt(2)
            ...
            return {"status": "sucesso", "tipo": "dunnett", "dms": round(dms_dunnett, 4), "testemunha": testemunha, "media_testemunha": round(media_test, 2), "resultados": res}
...
async def analisar_fatorial(file: UploadFile = File(...), tipo_teste: str = Form("anova"), modelo_regr: str = Form("linear"), testemunha: str = Form("")):
    try:
        ...
        if tipo_teste == "anova":
            return {
                "status": "sucesso", "cv": round((np.sqrt(qm_res)/mg)*100, 2), 
                "anova": [
        ...
        if "dunnett_" in tipo_teste:
            if not testemunha or testemunha not in medias.index: return {"status": "erro", "mensagem": f"Testemunha '{testemunha}' não encontrada."}
            media_test = medias[testemunha]
            dms_dunnett = stats.t.ppf(1 - (0.05 / (2 * (n_niveis - 1))), gl_res) * ep * np.sqrt(2)
            res = [{"Tratamento": str(n), "Média": round(v, 2), "Diferença": round(v - media_test, 2), "Sig": "Significativo (*)" if abs(v - media_test) >= dms_dunnett else "ns"} for n, v in medias.items() if n != testemunha]
            return {"status": "sucesso", "tipo": "dunnett", "dms": round(dms_dunnett, 4), "testemunha": testemunha, "media_testemunha": round(media_test, 2), "resultados": res}

    except Exception as e: raise HTTPException(status_code=500, detail=str(e))

# ================= MÓDULO 3: PARCELAS SUBDIVIDIDAS (AGORA COM POST-HOC) =================
...
async def analisar_parcelas(file: UploadFile = File(...), tipo_teste: str = Form("anova"), modelo_regr: str = Form("linear"), testemunha: str = Form("")):
    try:
        ...
        if tipo_teste == "anova":
            ...
            return {"status": "sucesso", "cv_a": round((np.sqrt(qm_erro_a)/mg)*100,2), "cv_b": round((np.sqrt(qm_erro_b)/mg)*100,2), "anova": anova}
        ...
        if qm_err_atual <= 0 or gl_err_atual <= 0:
            return {"status": "erro", "mensagem": f"O Erro {fator} é igual a zero, impossibilitando testes estatísticos."}
        ...
        if "regr_" in tipo_teste:
            ...
            anova_reg = [
                {"FV": f"Regressão ({modelo_regr.capitalize()})", "GL": gl_reg, "SQ": round(sq_reg_raw, 2), "QM": round(qm_reg, 2), "F Calc": round(f_reg, 2), "F Tab": calcular_f_tab(gl_reg, gl_err_atual), "Sig": determinar_sig_texto(1 - stats.f.cdf(f_reg, gl_reg, gl_err_atual), gl_reg, gl_err_atual)},
                {"FV": "Desvios da Regressão", "GL": gl_desvio, "SQ": round(sq_desvio_raw, 2), "QM": round(qm_desvio, 2), "F Calc": round(f_desvio, 2), "F Tab": calcular_f_tab(gl_desvio, gl_err_atual), "Sig": determinar_sig_texto(1 - stats.f.cdf(f_desvio, gl_desvio, gl_err_atual), gl_desvio, gl_err_atual)},
            ]
            ax.set_title(f"Ajuste {modelo_regr.capitalize()} - Fator {fator}")
            ax.grid(True, linestyle='--', alpha=0.6); ax.legend()
            b_png, b_pdf = io.BytesIO(), io.BytesIO()
        ...
        if "tukey_" in tipo_teste:
            q_crit = qsturng(0.95, n_niveis, gl_err_atual); dms = q_crit * ep
            return {"status": "sucesso", "tipo": "tukey", "q": round(q_crit, 4), "dms": round(dms, 4), "tabela": [{"Nível": str(n), "Média": round(v, 2)} for n, v in medias.items()]}
        
        if "duncan_" in tipo_teste:
            rp_duncan = {p: round(qsturng((0.95)**(p-1), p, gl_err_atual) * ep, 4) for p in range(2, n_niveis + 1)}
            return {"status": "sucesso", "tipo": "duncan", "alcances": rp_duncan, "tabela": [{"Nível": str(n), "Média": round(v, 2)} for n, v in medias.items()]}

        if "dunnett_" in tipo_teste:
            if not testemunha or testemunha not in medias.index: return {"status": "erro", "mensagem": f"Testemunha '{testemunha}' não encontrada."}
            media_test = medias[testemunha]
            dms_dunnett = stats.t.ppf(1 - (0.05 / (2 * (n_niveis - 1))), gl_err_atual) * ep * np.sqrt(2)
            res = [{"Tratamento": str(n), "Média": round(v, 2), "Diferença": round(v - media_test, 2), "Sig": "Significativo (*)" if abs(v - media_test) >= dms_dunnett else "ns"} for n, v in medias.items() if n != testemunha]
            return {"status": "sucesso", "tipo": "dunnett", "dms": round(dms_dunnett, 4), "testemunha": testemunha, "media_testemunha": round(media_test, 2), "resultados": res}
